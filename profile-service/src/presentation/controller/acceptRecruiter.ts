...
export const acceptRecruiterController = (dependencies: IDependencies) => {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await acceptRecruiterUseCase(dependencies).execute(
        data
      );
      ...
      const dataToAdmin={
        ...
        name:result.companyDetails?.recruiters?.find(recruiter => recruiter.email === data.userEmail)?.name,
